...
export function MessagesContainer({ messages, loading, error, messagesEndRef, onClearError }) {
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 scrollbar-dark">
      {messages.length === 0 ? (
        ...
        <>
          ...
          {loading && (messages.length === 0 || messages[messages.length - 1]?.type === 'user') && (
            <div className="flex justify-start px-2">
              <div className="max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg px-4 py-3 text-sm rounded-curve bg-[#303030] text-white rounded-[18px] rounded-bl-[3px]">
